...
public class BasicUtilities implements Function, Loadable, Predicate
{
    ...
    private static class f_use implements Function
    {
       public Scalar evaluate(String n, ScriptInstance si, Stack l)
       {
          File className = BridgeUtilities.getFile(l);
          Class bridge;

          try
          {
             if (className.getParentFile() != null)
             {
                URLClassLoader loader = new URLClassLoader(new URL[] { className.getParentFile().toURL() });
                bridge = Class.forName(className.getName(), true, loader);
             }
             else
             {
                bridge = Class.forName(className.getName());
             }
    ...
    private static class eval implements Function
    {
       public Scalar evaluate(String n, ScriptInstance i, Stack l)
       {
          ...
          try 
          {
             return SleepUtils.getScalar(i.getScriptEnvironment().evaluateStatement(code));
          }
    ...
    private static class expr implements Function
    {
       public Scalar evaluate(String n, ScriptInstance i, Stack l)
       {
          ...
          try 
          {
             return SleepUtils.getScalar(i.getScriptEnvironment().evaluateExpression(code));
          }
          catch (YourCodeSucksException ex)
          {
             i.getScriptEnvironment().flagError(ex.getMessage());
